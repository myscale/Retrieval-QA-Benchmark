...
@REGISTRY.register_transform("mcsa_prompt")
...
@REGISTRY.register_transform("mcsa_fewshot_mmlu")
class FewshotMultipleChoiceTransform(BaseTransform):
    ...
    @classmethod
    def convert_csv(cls, example_csv: str) -> Any:
        example_df: pd.DataFrame = pd.read_csv(
            example_csv, names=("Question", "A", "B", "C", "D", "Answer")
        )
    ...
    def transform_question(self, data: Dict[str, Any], **params: Any) -> str:
        return self.sep_chr.join([self.example_csv, data["question"]])
