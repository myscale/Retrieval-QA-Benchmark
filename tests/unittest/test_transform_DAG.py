...
def test_DAG_1(num_base):
    ...
    from os import path
    num_base = 20
    config = yaml.safe_load(open(path.join(path.dirname(path.realpath(__file__)),
                                           'assets/test_transform_DAG.yaml')))
    chain = TransformChainFactory(chain_config=config['evaluator']['transform_chain']).build()
    d = chain(QARecord(id='test1', question='*'*num_base, answer="answer for test 1", type="open"))
    assert (math.ceil((100 - num_base)/12)  + 1) * 12 + num_base == len(d.question), "Execution count mismatched"
